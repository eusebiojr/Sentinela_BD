#!/usr/bin/env python3
"""
Script FILTRO POR POIs - buscar pontos notáveis das últimas 5 horas
COM FILTRO POR NOME DE POIs - filtra apenas POIs específicos RRP e TLS
                ...
                if response.status == 200:
                    ...
                    for evento in eventos:
                        ...
                        # Verificar também POIs com caracteres quebrados
                            ...
                            eventos_filtrados.append(evento)
                        elif 'Campo Grande' in descricao_poi and ('Manutencao' in descricao_poi or 'Manutenção' in descricao_poi):
                            eventos_filtrados.append(evento)
                    
                    todos_eventos.extend(eventos_filtrados)
                    
                    if page == 1 and eventos_filtrados:
                        print(f"   📌 Eventos filtrados nesta página: {len(eventos_filtrados)}/{len(eventos)}")
...
def processar_eventos(eventos):
    ...
    for poi, count in pois_encontrados.most_common():
        if poi in POIS_RRP:
            ...
            categoria = 'RRP'
        elif 'Campo Grande' in poi and 'Manuten' in poi:
            categoria = 'Campo Grande'
        else:
            categoria = 'Outros'
...
def main():
    ...
    print(f"   • Manutenção Campo Grande (se existir)")
    print()

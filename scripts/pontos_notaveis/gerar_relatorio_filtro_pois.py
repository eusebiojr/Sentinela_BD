#!/usr/bin/env python3
"""
Script de TESTE - buscar pontos notáveis das últimas 5 horas
SEM FILTRO DE IDS - traz todos os eventos disponíveis
...
# SEM FILTRO DE IDs - Script de teste
...
def buscar_eventos_12hrs():
    """Busca eventos das últimas 5 horas SEM FILTRO DE IDS"""
    print(f"🔍 Buscando TODOS os eventos disponíveis (sem filtro de IDs)")
    ...
    params = {
        ...
        # SEM FILTRO DE IDS - removido customerChildIds
        "page": 1,
        "size": 1000,
        "sort": "updatedAt,desc"
    }
        ...
        while True:
            ...
            with urllib.request.urlopen(request, timeout=60) as response:
                if response.status == 200:
                    ...
                    # SEM FILTRO - adicionar todos os eventos
                    todos_eventos.extend(eventos)
                    
                    print(f"   • Página {page}/{total_pages}: {len(eventos)} eventos recebidos")
...
def processar_eventos(eventos):
    ...
    for evento in eventos:
        # SEM FILTRO - processar todos os eventos
...
def gerar_csv(todos_eventos):
    ...
    filename = f"pontos_notaveis_TESTE_SEM_FILTRO_{timestamp}.csv"
    ...
    for evento in todos_eventos:
        evento_final = {
            'Customer_ID': evento.get('Customer_Child_Id', ''),
            ...
            'Data_Atualizacao': evento.get('Data_Atualizacao', ''),
            'Vehicle_Id': evento.get('Vehicle_Id', ''),
            'Fence_Id': evento.get('Fence_Id', '')
        }
...
def gerar_relatorio(eventos):
    ...
    # Total de IDs únicos encontrados
    print(f"\n📊 TOTAL DE IDs ÚNICOS: {len(ids_presentes)}")
...
def main():
    """Função principal"""
    print("="*80)
    print("🚛 PONTOS NOTÁVEIS - TESTE SEM FILTRO - ÚLTIMAS 5 HORAS")
    print("="*80)
    
    agora = datetime.now(CAMPO_GRANDE_TZ)
    print(f"🕐 Horário atual: {agora.strftime('%d/%m/%Y %H:%M:%S')} (Campo Grande/MS)")
    print(f"🆔 Modo: SEM FILTRO DE IDs (trazendo todos os eventos)")
    print()
    
    # SEM FILTRO DE IDs neste script de teste
    
    try:
        # Buscar eventos
        print("🔄 Iniciando busca de eventos...")
        eventos_raw = buscar_eventos_12hrs()
